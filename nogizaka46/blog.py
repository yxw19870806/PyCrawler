        ...
        try:
            ...
            while not is_over:
                ...
                if blog_page is None:
                    print_error_msg(account_name + " 第%s页日志获取失败" % page_count)
                    tool.process_exit()
                if not blog_page:
                    print_error_msg(account_name + " 第%s页日志解析失败" % page_count)
                    tool.process_exit()

                blog_data_list = get_blog_data_list(blog_page)
                if len(blog_data_list) == 0:
                    print_error_msg(account_name + " 第%s页日志分组失败" % page_count)
                    tool.process_exit()
                ...
                for blog_data in blog_data_list:
                    ...
                    if blog_id is None:
                        print_error_msg(account_name + " 日志解析日志id失败，日志内容：%s" % blog_data)
                        tool.process_exit()
