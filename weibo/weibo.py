...
class Weibo(common.Robot):
    ...
    def main(self):
        ...
        for user_id in sorted(user_id_list.keys()):
            ...
            # if IS_SORT == 1:
            #     image_path = self.image_temp_path + "\\" + user_name
            # else:
            ...
            #     except:
            #         print_error_msg("在JSON数据：" + str(page) + " 中没有找到'total'字段, user id: " + str(user_id))
            #         break
            #
            #     try:
            ...
            #             print_error_msg("JSON数据['photo_list']：" + str(image_info) + " 不是一个字典, user id: " + str(user_id))
            #             continue
            ...
            #                     try_count += 1
            #                 if try_count >= 5:
            #                     print_error_msg("下载图片失败，用户ID：" + str(user_id) + ", 第" + str(image_count) +  "张，图片地址：" + image_url)
            #                     break
            #
            #         else:
            #             print_error_msg("在JSON数据：" + str(image_info) + " 中没有找到'pic_name'字段, user id: " + str(user_id))
            #
            #         # 达到配置文件中的下载数量，结束
