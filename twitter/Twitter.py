...
class Twitter(common.Robot):
    ...
    def main(self):
        ...
        for user_account in sorted(user_id_list.keys()):
            ...
            #         print_error_msg("无法获取相册信息: " + photo_page_url)
            #         break
            #     try:
            #         page = json.read(photo_page_data)
            #     except:
            #         print_error_msg("返回信息：" + str(photo_page_data) + " 不是一个JSON数据, account: " + user_account)
            #         break
            #
            #     if not isinstance(page, dict):
            #         print_error_msg("JSON数据：" + str(page) + " 不是一个字典, account: " + user_account)
            #         break
            #     if not page.has_key("has_more_items"):
            #         print_error_msg("在JSON数据：" + str(page) + " 中没有找到'has_more_items'字段, account: " + user_account)
            #         break
            #     if not page['has_more_items']:
            #         is_last_page = True
            #     if page.has_key("items_html") is False:
            #         print_error_msg("在JSON数据：" + str(page) + " 中没有找到'items_html'字段, account: " + user_account)
            #         break
            #
            ...
            #             image_count += 1
            #         else:
            #             print_error_msg("获取第" + str(image_count) + "张图片信息失败：" + user_account + "：" + image_url)
            #
            #         # 达到配置文件中的下载数量，结束
