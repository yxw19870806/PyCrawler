        ...
        try:
            ...
            while True:
                ...
                if media_page_return_code != 1:
                    print_error_msg(user_account + " 无法获取相册信息: " + media_page_url)
                    break
                try:
                    media_page = json.loads(media_page_response)
                except:
                    print_error_msg(user_account + " 返回信息：" + str(media_page_response) + " 不是一个JSON数据")
                    break

                if not isinstance(media_page, dict):
                    print_error_msg(user_account + " JSON数据：" + str(media_page) + " 不是一个字典")
                    break
                if "has_more_items" not in media_page:
                    print_error_msg(user_account + " 在JSON数据：" + str(media_page) + " 中没有找到'has_more_items'字段")
                    break
                if "items_html" not in media_page:
                    print_error_msg(user_account + " 在JSON数据：" + str(media_page) + " 中没有找到'items_html'字段")
                    break
                if "min_position" not in media_page:
                    print_error_msg(user_account + " 在JSON数据：" + str(media_page) + " 中没有找到'min_position'字段")
                    break
