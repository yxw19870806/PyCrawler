...
class Twitter(robot.Robot):
    ...
    def main(self):
        ...
        if os.path.exists(self.save_data_path):
            # account_id  image_count  video_count  last_tweet_id
        ...
        try:
            ...
            while not is_over:
                ...
                if len(tweet_list) == 0:
                    ...
                    tool.process_exit()
                tweet_id_find = re.findall('data-tweet-id="([\d]*)"', tweet_list[0])
                if len(tweet_id_find) != 1:
                    print_error_msg(account_id + " tweet id解析异常，tweet数据：" + tweet_list[0])
                    tool.process_exit()
                if first_tweet_id == "0":
                    first_tweet_id = tweet_id_find[0]

                # 视频
                if False and IS_DOWNLOAD_VIDEO == 1:
