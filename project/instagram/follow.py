...
def main():
    ...
    if "www.instagram.com" in all_cookie_from_browser:
        ...
        tool.process_exit()
    # 设置代理
    is_proxy = robot.analysis_config(config, "IS_PROXY", 2, 1)
    if is_proxy == 1 or is_proxy == 2:
        proxy_ip = robot.analysis_config(config, "PROXY_IP", "127.0.0.1", 0)
        proxy_port = robot.analysis_config(config, "PROXY_PORT", "8087", 0)
        # 使用代理的线程池
