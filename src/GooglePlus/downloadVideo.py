    ...
    def main(self):
        ...
        if not os.path.exists(videoUrlFileDir):
            if not self.createDir(videoUrlFileDir):
                self.printStepMsg(u"视频下载地址页面目录，创建文件夹：" + traceLogDir, self.isShowError, self.isLog)
                self.processExit()
            self.printStepMsg(u"视频下载地址页面目录不存在，创建文件夹：" + videoUrlFileDir, self.isShowError, self.isLog)
        # 视频url保存的html文件
        ...
        for userId in userIdList:
            ...
            self.trace(u"视频专辑地址：" + videoAlbumUrl, self.isShowError, self.isLog)
            ...
            if videoAlbumPage:
                ...
                self.printErrorMsg(u"无法获取视频首页: " + videoAlbumUrl, self.isShowError, self.isLog)
            # 生成下载视频url的文件
            if videoCount > 0:
        ...
        stopTime = time.time()
        self.printStepMsg(u"存档文件中所有用户视频地址已成功获取，耗时" + str(int(stopTime - startTime)) + u"秒，共计视频地址" + str(allVideoCount) + "个", self.isShowError, self.isLog)
